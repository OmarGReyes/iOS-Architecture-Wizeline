...
protocol SonCellDelegate: AnyObject {
    func didTapThreeDot()
}

final class SongCell: UITableViewCell, CustomConfigurableCell {
    ...
    func setup(with data: DataCellViewModable) {
        guard let song: SongCellModel = data as? SongCellModel else { return }
        nameLabel.text = song.name
        artistLabel.text = song.artist
    }
}
